...
export default function App() {
  ...
  const { resetDemo, adapterName } = useStore();
  const Comp = SCREENS.find((s) => s.id === active).comp;

  return (
    <>
      <Nav screens={SCREENS} active={active} onNav={setActive} />
      <div style={{ maxWidth: 1320, margin: '0 auto', padding: '10px 24px 0', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        <Legend />
        <span className="micro" style={{ letterSpacing: '0.04em' }}>Mockup of the ideal Jira workspace · data: {adapterName}</span>
